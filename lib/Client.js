    ...
    */
    getGuildAuditLog(guildID, options = {}) {
        ...
        return this.requestHandler.request("GET", Endpoints.GUILD_AUDIT_LOGS(guildID), true, options).then((data) => {
            ...
            const autoModerationRules = data.auto_moderation_rules.map((rule) => (
                {
                    ...
                    exemptUsers: rule.exempt_users,
                    guildID: rule.guild_id,
                    id: rule.id,
                    name: rule.name,
                    triggerMetadata: rule.trigger_metadata ? {
                        get allowList() {
                            emitDeprecation("AUTOMOD_CAMEL_CASE_TRIGGER_META");
                            return rule.trigger_metadata.allow_list;
                        },

                        get keywordFilter() {
                            emitDeprecation("AUTOMOD_CAMEL_CASE_TRIGGER_META");
                            return rule.trigger_metadata.keyword_filter;
                        },

                        get mentionTotalLimit() {
                            emitDeprecation("AUTOMOD_CAMEL_CASE_TRIGGER_META");
                            return rule.trigger_metadata.mention_total_limit;
                        },

                        get regexPatterns() {
                            emitDeprecation("AUTOMOD_CAMEL_CASE_TRIGGER_META");
                            return rule.trigger_metadata.regex_patterns;
                        },
