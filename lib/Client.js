...
class Client extends EventEmitter {
     ...
     * @param {Object} [options.voice] Options for voice connections
    ...
    constructor(token, options) {
         ...
         */
        this.voiceConnections = new VoiceConnectionManager(undefined, this.options.voice);
        delete this.options.voice;
    ...
    addInviteTargetUser(inviteID, userID) {
        return this.requestHandler.request("PUT", Endpoints.INVITE_TARGET_USER(inviteID, userID), true);
    }

    addInviteTargetUsers(inviteID, userIDs) {
        return this.requestHandler.request("POST", Endpoints.INVITE_TARGET_USERS_BULK_ADD(inviteID), {
            user_ids: userIDs
        });
    }
     ...
     * @param {Number} [options.flags] The flags of the channel
     ...
     * @param {Array<String>} [options.roleIDs] An array of role IDs to give to the users when they accept the invite
     * @param {Array<String>} [options.targetUserIDs] An array of user IDs that are able to accept this invite. If over 1000 users are provided, they will be processed in the background
     * @param {String} [reason] The reason to be displayed in audit logs
    ...
    createChannelInvite(channelID, options = {}, reason) {
        return this.requestHandler.request("POST", Endpoints.CHANNEL_INVITES(channelID), true, {
            ...
            target_user_ids: options.targetUserIDs?.length <= 1000 ? options.targetUserIDs : undefined,
            reason: reason
        }, options.targetUserIDs?.length > 1000
            ? [{
                    fieldName: "target_users_file",
                    file: Buffer.from(options.targetUserIDs.join("\n")),
                    name: "target_users.csv"
                }]
            : undefined).then((invite) => new Invite(invite, this));
    }
     ...
     * @param {Boolean} [content.attachments[].is_spoiler] Whether the attachment is a spoiler or not
     * @param {Number} [content.attachments[].duration_secs] The duration of the attachment in seconds (required for voice messages)
     * @param {String} [content.attachments[].waveform] The base64 encoded waveform of the attachment (required for voice messages)
     ...
     * @param {Role.RoleColors} [options.colors] The colors of the role
     * @param {Number} [options.colors.primaryColor] The primary color of the role
     * @param {Number?} [options.colors.secondaryColor] The secondary color of the role
     * @param {Number?} [options.colors.tertiaryColor] The tertiary color of the role
     ...
     * @param {Boolean} [options.message.attachments[].is_spoiler] Whether the attachment is a spoiler or not
     * @param {Number} [options.message.attachments[].duration_secs] The duration of the attachment in seconds (required for voice messages)
     * @param {String} [options.message.attachments[].waveform] The base64 encoded waveform of the attachment (required for voice messages)
     ...
     * @param {Number} [options.flags] The new flags for the channel that is moved
     ...
     * @param {Number} [channelPositions[].flags] The new flags of the channel
     * @returns {Promise}
     */
    ...
    /**
     * Edits a guild's incident actions
     * @param {String} guildID The ID of the guild
     * @param {Object} options The properties to edit
     * @param {Date?} [options.invitesDisabledUntil] When invites get enabled again. Set to `null` to lift the restriction
     * @param {Date?} [options.dmsDisabledUntil] When DMs get enabled again. Set to `null` to lift the restriction
     * @returns {Promise<Guild.IncidentsData>}
     */
    editGuildIncidentActions(guildID, options) {
        return this.requestHandler.request("PUT", Endpoints.GUILD_INCIDENT_ACTIONS(guildID), true, {
            ...
            invitesDisabledUntil: data.invites_disabled_until != null ? Date.parse(data.invites_disabled_until) : null,
            dmsDisabledUntil: data.dms_disabled_until != null ? Date.parse(data.dms_disabled_until) : null,
            dmSpamDetectedAt: data.dm_spam_detected_at != null ? Date.parse(data.dm_spam_detected_at) : null,
            raidDetectedAt: data.raid_detected_at != null ? Date.parse(data.raid_detected_at) : null
        }));
    }
     ...
     * @param {Object} options The properties to edit
     * @param {String?} [options.avatar] The new guild-level avatar as a base64 data URI. Available only when memberID is set to "@me". Note: base64 strings alone are not base64 data URI strings
     * @param {String?} [options.banner] The new guild-level banner as a base64 data URI. Available only when memberID is set to "@me". Note: base64 strings alone are not base64 data URI strings
     * @param {String?} [options.bio] The new guild-level member bio. Available only when memberID is set to "@me".
        ...
        return this.requestHandler.request("PATCH", Endpoints.GUILD_SCHEDULED_EVENT(guildID, eventID), true, {
            ...
            recurrence_rule: event.recurrenceRule !== undefined
                ? event.recurrenceRule !== null
                    ? {
                            start: event.recurrenceRule.start,
                            end: event.recurrenceRule.end,
                            frequency: event.recurrenceRule.frequency,
                            interval: event.recurrenceRule.interval,
                            by_weekday: event.recurrenceRule.byWeekday,
                            by_n_weekday: event.recurrenceRule.byNWeekday,
                            by_month: event.recurrenceRule.byMonth,
                            by_month_day: event.recurrenceRule.byMonthDay,
                            by_year_day: event.recurrenceRule.byYearDay,
    ...
    /**
     * Edits the list of users allowed to use the invite
     * @param {String} inviteID The ID of the invite
     * @param {Array<String>} userIDs An array of user IDs allowed to use this invite
     * @returns {Promise}
     */
    editInviteTargetUsers(inviteID, userIDs) {
        return this.requestHandler.request("PUT", Endpoints.INVITE_TARGET_USERS(inviteID), true, undefined, {
            fieldName: "target_users_file",
            file: Buffer.from(userIDs.join("\n")),
            name: "target_users.csv"
        });
    }
     ...
     * @param {String} [options.attachments[].description] A description for the attachment
     * @param {Boolean} [content.attachments[].is_spoiler] Whether the attachment is a spoiler or not
     * @param {Number} [content.attachments[].duration_secs] The duration of the attachment in seconds (required for voice messages)
     * @param {String} [content.attachments[].waveform] The base64 encoded waveform of the attachment (required for voice messages)
     * @param {Array<Object>} [options.components] An array of components. See [Discord's Documentation](https://discord.com/developers/docs/interactions/message-components#what-is-a-component) for object structure
     ...
     * @param {Boolean} [options.withComponents=false] Whether to allow sending messages with components for non-application webhooks
     * @returns {Promise<Message>}
     */
     ...
     * @param {Array<Object>} [options.attachments] The files to attach to the message
     * @param {Buffer} options.attachments[].file A buffer containing file data
     * @param {String} options.attachments[].filename What to name the file
     * @param {String} [options.attachments[].description] A description for the attachment
     * @param {Boolean} [content.attachments[].is_spoiler] Whether the attachment is a spoiler or not
     * @param {Number} [content.attachments[].duration_secs] The duration of the attachment in seconds (required for voice messages)
    ...
    /**
     * Gets the role member counts of a guild
     * @param {String} guildID The guild ID to fetch the role member counts for
     * @returns {Promise<Object<String, Number>>} An object mapping role IDs to the amount of members having the role
     */
    ...
    /**
     * Gets the target users of an invite
     * @param {String} inviteID The ID of the invite
     * @returns {Promise<Array<String>>} The list of users that are allowed to see and accept the invite
     */
    getInviteTargetUsers(inviteID) {
        return this.requestHandler.request("GET", Endpoints.INVITE_TARGET_USERS(inviteID), true).then((resp) => resp.trimEnd().split(/\r?\n/).slice(1));
    }

    /**
     * Gets the status of the job processing the invite target user list
     * @param {String} inviteID The ID of the invite
     * @returns {Promise<Object>} An [object](https://discord.com/developers/docs/resources/invite#get-target-users-job-status-example-response) containing the information about the job status
     */
    getInviteTargetUsersJobStatus(inviteID) {
        return this.requestHandler.request("GET", Endpoints.INVITE_TARGET_USERS_JOB_STATUS(inviteID), true);
    }
    ...
    getPins(channelID, options) {
        return this.requestHandler.request("GET", Endpoints.CHANNEL_PINS(channelID), true, options).then((pins) => ({
     ...
     * @param {Boolean} [options.daveEncryption=true] Whether to enable DAVE E2EE encryption for voice connections
    ...
    removeInviteTargetUser(inviteID, userID) {
        return this.requestHandler.request("DELETE", Endpoints.INVITE_TARGET_USER(inviteID, userID), true);
    }

    removeInviteTargetUsers(inviteID, userIDs) {
        return this.requestHandler.request("POST", Endpoints.INVITE_TARGET_USERS_BULK_DELETE(inviteID), true, {
            user_ids: userIDs
        });
    }
    ...
    /**
     * Searches through the messages of a guild matching a search query. Requires the `messageContent` intent.
     * @param {String} guildID The ID of the guild to search through
     * @param {Object} [options] The options for message search (described below)
     * @param {Number} [options.limit=25] The maximum number of results to return (max 25)
     * @param {Number} [options.offset=0] A number to offset the returned messages by (max 9975)
     * @param {String} [options.maxID] Get messages before this message ID
     * @param {String} [options.minID] Get messages after this message ID
     * @param {Number} [options.slop=2] The maximum number of words to skip between matching tokens in the search context (max 100)
     * @param {String} [options.content] Filters messages by message content
     * @param {Array<String>} [options.channelID] Filters messages by these channel IDs
     * @param {Array<String>} [options.authorType] Filters messages by author type (`user`/`bot`/`webhook`). Each of these can be negated by prefixing with `-`
     * @param {Array<String>} [options.authorID] Filters messages by these author IDs
     * @param {Array<String>} [options.mentions] Filters messages that mention these user IDs
     * @param {Array<String>} [options.mentionsRoleID] Filters messages that mention these role IDs
     * @param {Boolean} [options.mentionEveryone] Filters messages that do or do not mention everyone/here
     * @param {Array<String>} [options.repliedToUserID] Filters messages that reply to these user IDs
     * @param {Array<String>} [options.repliedToMessageID] Filters messages that reply to these message IDs
     * @param {Boolean} [options.pinned] Filters messages by whether they are pinned or not
     * @param {Array<String>} [options.has] Filters messages that have certain things (`image`/`sound`/`video`/`file`/`sticker`/`embed`/`link`/`poll`/`snapshot`). Each of these can be negated by prefixing with `-`
     * @param {Array<String>} [options.embedType] Filters messages that have embeds with these types (`image`/`video`/`gif`/`sound`/`article`)
     * @param {Array<String>} [options.embedProvider] Filters messages by embed provider name (case-sensitive; max 256 characters per entry, max 100 entries)
     * @param {Array<String>} [options.linkHostname] Filters messages by links with these hostnames (max 256 characters per entry, max 100 entries)
     * @param {Array<String>} [options.attachmentFilename] Filters messages by attachments file names (max 1024 characters per entry, max 100 entries)
     * @param {Array<String>} [options.attachmentExtension] Filters messages by attachment file extensions (max 256 characters per entry, max 100 entries)
     * @param {String} [options.sortBy] The sorting algorithm to use (`relevance`/`timestamp`)
     * @param {String} [options.sortOrder="desc"] The order to sort results in (`asc`/`desc`, works only with `timestamp` sorting)
     * @param {Boolean} [options.includeNSFW=false] Whether to include NSFW channels in the search or not
     * @returns {Promise<{
     * code: Number,
     * message: String,
     ...
     * }>} Either an error-like object asking to retry after some time if the guild is still unindexed, or an object with search results
     */
    searchGuildMessages(guildID, options = {}) {
        return this.requestHandler.request("GET", Endpoints.GUILD_MESSAGES_SEARCH(guildID), true, {
            ...
            mentions_role_id: options.mentionsRoleID,
            ...
            : {
                    ...
                    messages: data.messages.map((messages) => messages.map((message) => new Message(message, this))),
                    threads: data.threads?.map((thread) => Channel.from(thread, this)),
                    members: data.members?.map((member) => new ThreadMember(member, this))
                });
    }
     ...
     * @param {String} channelID The ID of the voice channel
     * @param {Object} options The voice channel status options
     * @param {String?} options.status The status to set
     * @param {String} [options.reason] The reason to be setting the voice status
     * @returns {Promise}
     */
    setVoiceChannelStatus(channelID, options) {
        return this.requestHandler.request("PUT", Endpoints.CHANNEL_VOICE_STATUS(channelID), true, options);
    }
