...
class Client extends EventEmitter {
    ...
    * @arg {String} [options.gateway.gatewayURL] Overrides the gateway URL that Dysnomia will use to connect to Discord's gateway
    ...
    */
    async connect() {
        ...
        try {
            let data;
            if(this.options.gateway.gatewayURL) {
                data = {url: this.options.gateway.gatewayURL};
            } else {
                data = await (this.shards.options.maxShards === "auto" || (this.shards.options.shardConcurrency === "auto" && this.bot) ? this.getBotGateway() : this.getGateway());
            }
