...
class Shard extends EventEmitter {
     ...
     * @param {Error} [error] The error that causes the disconnect
     */
    ...
    requestGuildChannelInfo(guildID, options) {
        if(Object.hasOwn(this.requestChannelInfoPromise, guildID)) {
            return new Error("A guild channel info request is already pending");
        }
        ...
        this.sendWS(Constants.GatewayOPCodes.REQUEST_CHANNEL_INFO, opts);
        return new Promise((res) => this.requestChannelInfoPromise[guildID] = {
            res: res,
            timeout: setTimeout(() => {
                res([]);
                delete this.requestChannelInfoPromise[guildID];
    ...
    reset() {
        ...
        this.requestChannelInfoPromise = {};
        ...
        switch(packet.t) { // (╯°□°）╯︵ ┻━┻
            ...
            case "PRESENCE_UPDATE": {
                if(packet.d.user.username !== undefined) {
                    ...
                    let oldUser = null;
                    if(user && (user.username !== packet.d.user.username
                        || user.discriminator !== packet.d.user.discriminator
                        || user.avatar !== packet.d.user.avatar
                        || user.globalName !== packet.d.user.global_name
                        || user.avatarDecorationData?.asset !== packet.d.user.avatar_decoration_data?.asset
                        || user.collectibles?.nameplate?.sku_id !== packet.d.user.collectibles?.nameplate?.sku_id
                        || user.primaryGuild?.identityEnabled !== packet.d.user.primary_guild?.identity_enabled
                        || user.primaryGuild?.identityGuildID !== packet.d.user.primary_guild?.identity_guild_id
                        || user.primaryGuild?.tag !== packet.d.user.primary_guild?.tag
                        || user.primaryGuild?.badge !== packet.d.user.primary_guild?.badge
                    )) {
                        oldUser = {
                    ...
                    if(!user || oldUser) {
                         ...
                         * @prop {Object?} oldUser.avatarDecorationData The data of the user's avatar decoration, or null if there is no avatar decoration
                         * @prop {User.Collectibles?} oldUser.collectibles The collectibles of the user, or null if there are no collectibles
                         * @prop {User.PrimaryGuild?} oldUser.primaryGuild The primary guild of the user, or null if there is no primary guild
                         */
            ...
            case "VOICE_CHANNEL_STATUS_UPDATE": {
                 ...
                 * @prop {Object} event The update event fired
                 * @prop {Guild | Object} event.guild The guild the status event was sent in. If the guild is not cached, this will be an object with an `id` key. No other property is guaranteed
                 * @prop {TextVoiceChannel | StageChannel | Object} event.channel The voice channel the status was set on. If the channel is not cached, this will be an object with an `id` key. No other property is guaranteed
                 * @prop {String?} event.status The status that was set on the channel
                 */
                this.emit("voiceChannelStatusUpdate", {
                    channel: channel,
                    guild: guild,
            ...
            case "VOICE_CHANNEL_START_TIME_UPDATE": {
                 ...
                 * @prop {Guild | Object} event.guild The guild the status event was sent in. If the guild is not cached, this will be an object with an `id` key. No other property is guaranteed
                 * @prop {TextVoiceChannel | StageChannel | Object} event.channel The voice channel the status was set on. If the channel is not cached, this will be an object with an `id` key. No other property is guaranteed
                 * @prop {Number?} event.voiceStartTime The UNIX timestamp of when the voice session started
                 */
                this.emit("voiceChannelStartTimeUpdate", {
                    channel: channel,
                    guild: guild,
                    voiceStartTime: packet.d.voice_start_time
                });
                break;
            }
            case "TYPING_START": {
                ...
                if(message) {
                    ...
                    if(reactionObj) {
                        ...
                        if(packet.d.user_id === this.client.user.id) {
                            if(packet.d.burst) {
                                reactionObj.meBurst = true;
                            } else {
                                reactionObj.me = true;
                            }
                        }
                        reactionObj.burstColors = packet.d.burst_colors;
                    } else {
            ...
            case "GUILD_MEMBER_UPDATE": {
                 ...
                 * @prop {Number} oldMember.flags The guild member flag bit set
            ...
            case "GUILD_ROLE_UPDATE": {
                 ...
                 * @prop {Role.RoleColors} oldRole.colors The colors of the role
            ...
            case "GUILD_SOUNDBOARD_SOUND_DELETE": {
                 ...
                 */
                this.emit("guildSoundboardSoundDelete", guild?.soundboardSounds.remove(packet.d) || {
                    id: packet.d.id,
                    guild: guild || {
                        id: packet.d.guild_id
                    }
            ...
            case "CHANNEL_INFO": {
                const guild = this.client.guilds.get(packet.d.guild_id);
                const channelInfo = packet.d.channels;

                /**
                 * Fired when Discord sends a guild's ephemeral channel data. Sent in response to `fetchChannelInfo()`
                 * @event Client#channelInfo
                 * @prop {Guild} guild The ID of the guild. If the guild is uncached, this will be an object with an `id` key. No other properties are guaranteed
                 * @prop {Array<Object>} channelInfo The guild's ephemeral [channel data](https://docs.discord.com/developers/events/gateway-events#channel-info)
                 */
                this.emit("channelInfo", guild || {id: packet.d.guild_id}, channelInfo);
                this.lastHeartbeatAck = true;

                if(Object.hasOwn(this.requestChannelInfoPromise, packet.d.guild_id)) {
                    clearTimeout(this.requestChannelInfoPromise[packet.d.guild_id].timeout);
                    this.requestChannelInfoPromise[packet.d.guild_id].res(channelInfo);
                    delete this.requestChannelInfoPromise[packet.d.guild_id];
                }
                break;
            }
