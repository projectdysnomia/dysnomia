...
class OggOpusTransformer extends BaseTransformer {
    ...
    _transform(chunk, enc, cb) {
        chunk = this._remainder ? Buffer.concat([this._remainder, chunk]) : chunk;
        let offset = 0;
        while (offset < chunk.length) {
        const ret = this.process(chunk.slice(offset));
        if(ret) {
            this._remainder = chunk.slice(offset);
            if(ret instanceof Error) this.emit("error", ret);
                return cb();
        }
        offset += this.chunk.length;
    }
    this._remainder = null;
    return cb();
    }
