...
const MAX_WS_BUFFER_ALLOC_SIZE = 5120;
...
class VoiceConnection extends EventEmitter {
     ...
     * @type {import("@snazzah/davey").DAVESession?}
     */
    daveSession = null;
    frameDuration = 20;
    /**
    ...
    constructor(id, options = {}) {
        ...
        this.daveEnabled = options.daveEncryption ?? true;
        if(!Davey && this.daveEnabled) {
            try {
                Davey = require("@snazzah/davey");
            } catch{
                // Error loading @snazzah/davey, DAVE E2EE not available
                this.daveEnabled = false;
            }
        }
    ...
    connect(data) {
        ...
        this.ws = new WebSocket(this.endpoint.href, this.wsOptions);
        /**
        ...
        this.ws.on("open", () => {
            ...
            if(this.reconnecting && data.session_id === this.sessionID && data.token === this.token && lastEndpoint === this.endpoint.hostname) {
                ...
                this.sendWS(VoiceOPCodes.IDENTIFY, {
                    max_dave_protocol_version: this.daveEnabled ? Davey?.DAVE_PROTOCOL_VERSION ?? 0 : 0,
                    server_id: this.id,
        ...
        this.ws.on("message", (/** @type {Buffer} */ m, isBinary) => {
            if(isBinary) {
                if(m instanceof ArrayBuffer) {
                    m = Buffer.from(m);
                } else if(Array.isArray(m)) { // Fragmented messages
                    m = Buffer.concat(m); // Copyfull concat is slow, but no alternative
                }
                const seq = m.readUInt16BE(0);
                this.wsSequence = seq;

                const op = m.readUInt8(2);
                switch(op) {
                    case VoiceOPCodes.MLS_EXTERNAL_SENDER: {
                    ...
                    case VoiceOPCodes.MLS_ANNOUNCE_COMMIT_TRANSITION: {
                        ...
                        try {
                            this.daveSession.processCommit(m.subarray(5));
                            if(transitionID !== 0) {
            ...
            switch(packet.op) {
                ...
                case VoiceOPCodes.SESSION_DESCRIPTION: {
                    ...
                    this.reconnecting = false;
                    this.ready = true;
                    this.#reinitDaveSession(data.user_id, data.channel_id);
                ...
                case VoiceOPCodes.CLIENTS_CONNECT: {
                    packet.d.user_ids.forEach((id) => this.#clientsConnected.add(id));
                    /**
                ...
                case VoiceOPCodes.CLIENT_DISCONNECT: {
                    ...
                    this.#clientsConnected.delete(packet.d.user_id);
                ...
                case VoiceOPCodes.DAVE_PREPARE_TRANSITION: {
                    ...
                    if(packet.d.transition_id === 0) {
                        ...
                        if(packet.d.protocol_version === 0) {
                            // Setting the passthrough expiry to 30 seconds, transitions max out at around 3 seconds, so this should be enough.
                ...
                case VoiceOPCodes.DAVE_EXECUTE_TRANSITION: {
                    ...
                    const transitioned = this.#executePendingTransition(packet.d.transition_id);
                    if(transitioned) {
                        /**
                         * Fired when the DAVE session transitions to a new epoch
                         * @event VoiceConnection#transitioned
                         * @prop {Number} transitionID The ID of the transition
                         */
    ...
    #reinitDaveSession(userID, channelID) {
        if(this.daveProtocolVersion > 0) {
            if(this.daveSession) {
                this.daveSession.reinit(this.daveProtocolVersion, userID, channelID);
                this.emit("debug", `DAVE session reinitialized for protocol version ${this.daveProtocolVersion}`);
            } else {
                this.daveSession = new Davey.DAVESession(this.daveProtocolVersion, userID, channelID);
            ...
            const daveSessionExisted = !!this.daveSession;
            this.daveSession?.reset();
            if(daveSessionExisted) {
    ...
    /**
     * Get the verification code between the bot and a user in the session.
     * @param {String} userID The ID of the user to get the verification code of
     */
    async getVerificationCode(userID) {
        ...
        return this.daveSession.getVerificationCode(userID);
    }
        ...
        this.udpSocket.on("message", (msg) => {
            ...
            if(this.daveSession && !data.equals(SILENCE_FRAME)) {
                if(this.daveSession.ready && !userID) {
                    ...
                    dropPacket = true;
                } else if(!this.daveSession.ready && this.daveProtocolVersion !== 0) {
                    this.emit("warn", "Packet was dropped due to DAVE session not being ready yet");
                    dropPacket = true;
                } else if(userID) {
                    try {
            ...
            if(this.receiveStreamOpus) {
                 ...
                 */
                this.receiveStreamOpus.emit("data", data, userID, msg.readUIntBE(4, 4), msg.readUIntBE(2, 2));
            }
            if(this.receiveStreamPCM) {
    ...
    sendWSBinary(op, data) {
        if(this.ws?.readyState === WebSocket.OPEN) {
            const buffer = data.length > MAX_WS_BUFFER_ALLOC_SIZE - 1 ? Buffer.allocUnsafe(1 + data.length) : (this.#wsSendBuffer ??= Buffer.allocUnsafe(MAX_WS_BUFFER_ALLOC_SIZE));
            data.copy(buffer, 1);
            buffer.writeUInt8(op);
            this.ws.send(buffer.subarray(0, 1 + data.length));
        }
    }
