...
class VoiceConnection extends EventEmitter {
        ...
        this.ws.on("message", (/** @type {Buffer} */ m, isBinary) => {
            if(isBinary) {
                ...
                switch(op) {
                    ...
                    case VoiceOPCodes.MLS_PROPOSALS: {
                        ...
                        const {commit, welcome} = this.daveSession.processProposals(optype, m.subarray(4));
                        if(commit) {
                    ...
                    case VoiceOPCodes.MLS_ANNOUNCE_COMMIT_TRANSITION: {
                        ...
                        try {
                            ...
                            this.emit("debug", "MLS commit processed");
                        } catch(e) {
                    ...
                    case VoiceOPCodes.MLS_WELCOME: {
                        ...
                        try {
                            ...
                            this.sendWS(VoiceOPCodes.DAVE_TRANSITION_READY, {transition_id: transitionId});
                            this.emit("debug", "MLS welcome processed");
                        } catch(e) {
            ...
            switch(packet.op) {
                ...
                case VoiceOPCodes.DAVE_PREPARE_TRANSITION: {
                    this.emit("debug", `Preparing for DAVE transition (${packet.d.transition_id})`);
    ...
    #executePendingTransition(transitionId) {
        if(!this.#davePendingTransition) {
            return this.emit("warn", `Recieved execute transition, but we dont have a pending transition (${transitionId})`);
        }
        let transitioned = false;
        if(transitionId !== this.#davePendingTransition.transition_id) {
            this.emit("warn", `Recieved execute transition for an unexpected transition id (expected: ${this.#davePendingTransition.transition_id}, actual: ${transitionId})`);
        } else {
    ...
    #daveReady() {
        return this.daveSession?.ready && this.#davePendingTransition?.protocol_version !== 0;
    }
    ...
    registerReceiveEventHandler() {
        this.udpSocket.on("message", (msg) => {
            ...
            if(this.#daveReady() && userID && data !== SILENCE_FRAME) {
                try {
    ...
    sendWSBinary(op, data) {
        if(this.ws?.readyState === WebSocket.OPEN) {
            this.#wsSendBuffer ??= Buffer.allocUnsafe(1024);
            data.copy(this.#wsSendBuffer, 1);
            this.#wsSendBuffer.writeUInt8(op);
            this.ws.send(this.#wsSendBuffer.subarray(0, 1 + data.length));
        }
    }
    ...
    _sendAudioFrame(unencryptedFrame) {
        ...
        const frame = this.#daveReady() && unencryptedFrame !== SILENCE_FRAME ? this.daveSession.encryptOpus(unencryptedFrame) : unencryptedFrame;
