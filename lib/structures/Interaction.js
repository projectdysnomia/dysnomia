...
class Interaction extends Base {
    ...
    constructor(data, client) {
        ...
        if(data.guild !== undefined) {
            const guild = client.guilds.get(data.guild.id);

            if(guild) {
                // for some reason, Discord sends the preferred guild locale under the `locale` key
                data.guild.preferred_locale ??= data.guild.locale;
                guild.update(data.guild);

                /**
                 * The guild the interaction was created in. Can be partial only with the `id`, `locale`, and `features`.
                 * @type {Guild?}
                 */
                this.guild = guild;
            } else {
             ...
             * @deprecated Use the `id` from {@link Interaction#guild} instead
             * @type {String?}
             */
             ...
             * @deprecated Use the `preferredLocale` from {@link Interaction#guild} instead
             * @type {String?}
             */
            this.guildLocale = data.guild.locale;
        }
        ...
        if(data.member !== undefined) {
            if(this.channel.guild) {
                ...
                const guild = this.#client.guilds.get(data.guild.id);
