...
class Invite extends Base {
    ...
    constructor(data, client) {
        ...
        if(data.roles !== undefined) {
            /**
             * A collection of roles that will be given to users if they join using this invite
             * @type {Collection<Role>?}
             */
            this.roles = new Collection(Role);
            for(const role of data.roles) {
                const roleObject = this.guild?.roles.update(role, this.guild) ?? new Role(role, this.guild);
                this.roles.add(roleObject);
            }
        }
    }

    addTargetUser(userID) {
        return this.#client.addInviteTargetUser.call(this.#client, this.code, userID);
    }

    addTargetUsers(userIDs) {
        return this.#client.addInviteTargetUsers.call(this.#client, this.code, userIDs);
    }
    ...
    /**
     * Edits the list of users allowed to use this invite
     * @param {Array<String>} userIDs An array of user IDs allowed to use this invite
     * @returns {Promise}
     */
    editTargetUsers(userIDs) {
        return this.#client.editInviteTargetUsers.call(this.#client, this.code, userIDs);
    }

    /**
     * Gets the target users of this invite
     * @returns {Promise<Array<String>>} The list of users that are allowed to see and accept the invite
     */
    getTargetUsers() {
        return this.#client.getInviteTargetUsers.call(this.#client, this.code);
    }

    /**
     * Gets the status of the job processing this invite's target user list
    ...
    getTargetUsersJobStatus() {
        return this.#client.getInviteTargetUsersJobStatus.call(this.#client, this.code);
    }

    removeTargetUser(userID) {
        return this.#client.removeInviteTargetUser.call(this.#client, this.code, userID);
    }

    removeTargetUsers(userIDs) {
        return this.#client.removeInviteTargetUsers.call(this.#client, this.code, userIDs);
    }
