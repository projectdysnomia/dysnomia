...
* @prop {String?} avatarDecoration The hash of the user's avatar decoration, or null if no avatar decoration
* @prop {String?} avatarDecorationURL The URL of the user's avatar decoration which will be a PNG
...
class User extends Base {
    ...
    get avatarDecorationURL() {
        ...
        const isPreset = this.avatarDecoration.startsWith("v2_");
        const url = isPreset
            ? Endpoints.USER_AVATAR_DECORATION_PRESET(this.avatarDecoration)
            : Endpoints.USER_AVATAR_DECORATION(this.id, this.avatarDecoration);
        return this.#client._formatImage(url, "png");
    }
    ...
    * @arg {String} [format] The filetype of the decoration ("jpg", "jpeg", "png", "gif", or "webp")
    * @arg {Number} [size] The size of the decoration (any power of two between 16 and 4096)
    * @returns {String?}
    */
    dynamicAvatarDecorationURL(format, size) {
        ...
        const url = isPreset
            ? Endpoints.USER_AVATAR_DECORATION_PRESET(this.avatarDecoration)
            : Endpoints.USER_AVATAR_DECORATION(this.id, this.avatarDecoration);
        return this.#client._formatImage(url, format, size);
    }
