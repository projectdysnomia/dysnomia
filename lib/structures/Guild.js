...
class Guild extends Base {
    ...
    update(data) {
        ...
        if(data.incidents_data !== undefined) {
            /**
             * Data about incidents in the guild
             * @type {Guild.IncidentsData?}
             */
            this.incidentsData = data.incidents_data != null
                ? {
                        invitesDisabledUntil: data.incidents_data.invites_disabled_until != null ? Date.parse(data.incidents_data.invites_disabled_until) : null,
                        dmsDisabledUntil: data.incidents_data.dms_disabled_until != null ? Date.parse(data.incidents_data.dms_disabled_until) : null,
                        dmSpamDetectedAt: data.incidents_data.dm_spam_detected_at != null ? Date.parse(data.incidents_data.dm_spam_detected_at) : null,
                        raidDetectedAt: data.incidents_data.raid_detected_at != null ? Date.parse(data.incidents_data.raid_detected_at) : null
                    }
                : null;
        }
    }
    ...
    editIncidentActions(options) {
        return this.#client.editGuildIncidentActions.call(this.#client, this.id, options);
    }
    ...
    /**
     * Request the guild's ephemeral channel data
     * @param {Object} options Options for fetching the channel info
     * @param {Array<String>} options.fields The fields to fetch from the channel info
     ...
     * @returns {Promise<Array<Object>>} Resolves with the fetched [channel data](https://docs.discord.com/developers/events/gateway-events#channel-info)
     */
    fetchChannelInfo(options) {
        return this.shard.requestGuildChannelInfo(this.id, options);
    }
    ...
    /**
     * Gets the role member counts of this guild
     * @returns {Promise<Object<String, Number>>} An object mapping role IDs to the amount of members having the role
     */
    getRoleMemberCounts() {
        return this.#client.getGuildRoleMemberCounts.call(this.#client, this.id);
    }
    ...
    /**
     * Searches through the messages of this guild matching a search query. Requires the `messageContent` intent.
    ...
    searchMessages(options) {
        return this.#client.searchGuildMessages.call(this.#client, this.id, options);
    }
 ...
 */
/**
 * The incident data for a guild
 * @typedef Guild.IncidentsData
 * @prop {Number?} invitesDisabledUntil The timestamp when invites get enabled again
 * @prop {Number?} dmsDisabledUntil The timestamp when direct messages get enabled again
 * @prop {Number?} dmSpamDetectedAt The timestamp when the direct message spam was detected
 * @prop {Number?} raidDetectedAt The timestamp when the raid was detected
 */
