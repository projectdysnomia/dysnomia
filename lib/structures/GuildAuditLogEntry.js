...
class GuildAuditLogEntry extends Base {
    ...
    constructor(data, guild) {
        ...
        if(data.options) {
            ...
            if(data.options.status) {
                /**
                 * The new voice channel status, action types 192 (VOICE\_CHANNEL\_STATUS\_UPDATE) only
                 * @type {String?}
                 */
    ...
    get target() { // pay more, get less
        if(this.actionType < 10) { // Guild
            return this.guild;
        } else if(this.actionType < 20 || this.actionType === AuditLogActions.VOICE_CHANNEL_STATUS_CREATE || this.actionType === AuditLogActions.VOICE_CHANNEL_STATUS_DELETE) { // Channel
            ...
            return null;
        } else if(this.actionType < 150) { // Auto Moderation Actions
            return this.guild?.shard.client.users.get(this.targetID);
        } else {
